(function (exports) {
  ...
  function drawChart() {
    ...
    var book1BarNodes = book1Bars.selectAll(".bar").data(chartData);
    ...
    var connectionNodes = connections.selectAll("path").data(chartData);
    ...
    var book2BarNodes = book2Bars.selectAll(".bar").data(chartData);
    ...
    // - Append Brush
    brushG.call(brushHandle).select(".overlay").on("dblclick", restoreCanvas);
  ...
  function updateChart(duration) {
    ...
    connections
      ...
      .attr("d", function (d) {
        return (
          "M " +
          xScale(d.book1_chunk) +
          " 150 C " +
          xScale(d.book1_chunk) +
          " 250," +
          xScale(d.book2_chunk) +
          " 220 , " +
          xScale(d.book2_chunk) +
          " " +
          300
    ...
    x0Axis.tickValues(
      selectedLine ? [1, selectedLine.book1_chunk, max.book1] : [1, max.book1]
    );
    x0ScaleNode
      .transition(t)
      .call(x0Axis)
    ...
    x1Axis.tickValues(
      selectedLine ? [1, selectedLine.book2_chunk, max.book2] : [1, max.book2]
    );
    x1ScaleNode
      .transition(t)
      .call(x1Axis)
    ...
    marksG
      ...
      .attr("y2", function (d) {
        return d.yScale(config.chunk_size) + d.y;
      });
  ...
  function makeLinesVisible() {
    ...
    getBars().attr("stroke-width", barWidth).attr("opacity", null);
